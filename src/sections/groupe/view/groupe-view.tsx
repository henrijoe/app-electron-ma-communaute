...
export function GroupeView() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const payload = {
        libelleGroupe: data.libelleGroupe?.trim() || '',
        descriptionGroupe: data.descriptionGroupe || '',
        responsableGroupe: data.responsableGroupe || '',
      ...
      if (isEditMode && data.idGroupe) {
        ...
        const response = await apiClient.updateGroupe(updatedPayload);
        if (response.status === 1) {
          dispatch(setDataModifiesGroupe(updatedPayload));
    ...
    <DashboardContent>
      ...
      >
        ...
        <Box display="flex" gap={1.25} alignItems="center" sx={{ width: { xs: '100%', md: 'auto' }, justifyContent: { xs: 'flex-start', md: 'flex-end' } }}>
          ...
          {!isDesktopApp && canManageGroupe && (
            <>
              <Tooltip title="Ajouter groupe">
                <IconButton
                  color="primary"
              ...
              <Card key={row.idGroupe} variant="outlined" sx={{ p: 1.75, borderRadius: 2, boxShadow: 'none' }}>
                ...
                <Grid container spacing={1.25}>
                  ...
                  <Grid item xs={6}>
                    ...
                    <ContactPhoneLink fallback="-" value={responsableContactByName.get(row.responsableGroupe)} />
                  </Grid>
                </Grid>
                ...
                {canManageGroupe && (
                  <Box display="flex" justifyContent="flex-end" gap={1} mt={1.5}>
                    <Tooltip title="Modifier">
                      <IconButton color="primary" size="small" onClick={() => handleEditGroupe(row)}>
                        <Iconify icon="solar:pen-bold" />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title={deleteLoading ? 'Suppression...' : 'Supprimer'}>
                      <span>
                        <IconButton size="small" color="error" disabled={deleteLoading} onClick={() => handleDeleteGroupe(row.idGroupe)}>
